...
def _identify_student(text_by_page: list, filename: str) -> dict:
    ...
    for pat in [
        ...
        r'(МИНИСТЕРСТВ\w+[^\n]+)',
        # Вуз, названный на титуле аббревиатурой: «РГУ нефти и газа (НИУ)
        # имени И.М. Губкина». Слова «университет» в такой строке нет, и
        # раньше организация со всего листа не вычитывалась вовсе.
        r'([^\n]{0,60}\bим(?:ени)?\.?\s*[А-ЯЁ]\.\s*[А-ЯЁ]\.[^\n]{0,60})',
    ...
    if m:
        ...
        # Бланк «Москва, 20__»: две последние цифры вписаны в линейку, и в
        # тексте между ними остаются её подчёркивания или пробел –
        # «Москва, 20__2_5___». Запасным ходом, а не первым: в дате
        # «20 05 2025» четыре цифры подряд – единственный верный год.
        m = re.search(r'(?<!\d)20[\s_]{0,8}(\d)[\s_]{0,8}(\d)(?!\d)',
                      title_text)
        if m:
            student['year'] = '20' + m.group(1) + m.group(2)
